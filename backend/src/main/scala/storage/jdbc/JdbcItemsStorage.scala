...
case class JdbcItemsStorage(pool: ZConnectionPool) extends ItemsStorage with JdbcStorage[Item] {
  ...
  override implicit val schema: Schema[Item] =
...
object JdbcItemsStorage {
  ...
  private def selectAllQuery(implicit jdbcDecoder: JdbcDecoder[Item]) = transaction {
    sql"SELECT * FROM item".query[Item].selectAll
  }

  private def selectItemByIdQuery(id: Int)(implicit jdbcDecoder: JdbcDecoder[Item]) = transaction {
    sql"SELECT * FROM item WHERE id = $id".query[Item].selectOne
  }

  private def selectItemsByTagIdQuery(tagId: Int)(implicit jdbcDecoder: JdbcDecoder[Item]) = transaction {
